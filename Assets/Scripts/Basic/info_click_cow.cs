...
public class info_click_cow : MonoBehaviour
{
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.transform.gameObject == transform.gameObject) //info_icon 클릭
                {
                    ...
                    hpMove("hungry_hp", hungry_pre, hungry_pre - cow.hungry);
                    hpMove("poop_hp", poop_pre, poop_pre - cow.hungry);
                    hpMove("play_hp", play_pre, play_pre - cow.hungry);
    ...
    public void hpMove(string tag, int value, int delta)    //hp바 동작 구현
    {
        ...
        hpbar_tmp = hpbar_sx / cow.valueMax;   //최대 체력에 따른 hp바 이동량 설정

        if (delta < 0)
        {
            float move = ((cow.valueMax - value) + delta) * hpbar_tmp; //hp바 이동할 크기
            value -= delta; //hp 재설정
        ...
        if (delta > 0)
        {
            if (value + delta > cow.valueMax)
                delta = (cow.valueMax - value);
