...
public class Chicken_Attack : MonoBehaviour
{
    ...
    public bool Chicken_Basic_Attack()
    {
        ...
        if (!is_Attack)//닿지 않았다면 계속 가기 닿으면 그만 
        {
            ...
            {
                is_target_cow = true;
                transform.position = Vector3.MoveTowards(transform.position, cow_enemy.transform.position, 0.2f);
            }
            else if (min_distance == chicken_distance)
            {
                is_target_chicken = true;
                transform.position = Vector3.MoveTowards(transform.position, chicken_enemy.transform.position, 0.2f);
            }
            else if (min_distance == tiger_distance)
            {
                is_target_tiger = true;
                transform.position = Vector3.MoveTowards(transform.position, tiger_enemy.transform.position, 0.2f);
            }
        }
        else//닿았다면 뒤로 간다음 다시 가게 하면 
        {
            ...
            if(is_target_cow)
            {
                if(cow_enemy.transform.position.x <transform.position.x)//소가 왼쪽이라면 -> 오른쪽으로 가기
                {
                    ...
                    //is_Attack = false;//근접 공격 false로 바꿔줌 
                }
                else//왼쪽으로 가기 
                {
                    Vector3 pos = new Vector3(cow_enemy.transform.position.x - back_distance, cow_enemy.transform.position.y, cow_enemy.transform.position.z);
            ...
            {
                if (chicken_enemy.transform.position.x < transform.position.x)//소가 왼쪽이라면 -> 오른쪽으로 가기
                {
            ...
            {
                if (tiger_enemy.transform.position.x < transform.position.x)//소가 왼쪽이라면 -> 오른쪽으로 가기
                {
    ...
    public bool Chicken_Find_Target()
    {
        if(!is_find_target)//target을 찾지 않았다면 
        {
            ...
            if (chicken_enemy != null)
            {
                ...
                //Debug.Log("닭이랑 거리: " + chicken_distance);
            ...
            if (tiger_enemy != null)
            {
                ...
                //Debug.Log("호랑이랑 거리: " + tiger_distance);
